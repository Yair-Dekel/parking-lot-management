...
constexpr uint32_t ENTRY_REQUEST_PAYLOAD_SIZE = sizeof(uint32_t) * 3 + sizeof(uint8_t) * 2;
constexpr std::size_t PARKING_LOT_ID_OFFSET = 4;
constexpr std::size_t GATE_ID_OFFSET = 8;
constexpr std::size_t WANTS_HANDICAP_OFFSET = 12;
constexpr std::size_t WANTS_ELECTRIC_OFFSET = 13;
...
namespace parkpulse
{
    ...
    private:
        ...
        void handle_message(const char *payload, int client_fd);
    ...
    private:
        ...
        std::unordered_map<int, std::vector<char>> input_buffers_; // Ron: mapping client_fds to buffers
